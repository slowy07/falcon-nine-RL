...
def calculate_returns(next_value, rewards, masks, gamma=0.09):
    R = next_value
    returns = []
...
class MLP(nn.Module):
    def __init__(self, input_dim, output_dim):
        super().__init__()
        self.mapping = PositionMapping(input_dim=input_dim, L=7)

        h_dim = 128
        self.linear1 = nn.Linear(
            in_features=self.mapping.output_dim, out_features=h_dim, bias=True
        )
        ...
        if deterministic:
            ...
            if random.random() < exploration:
                ...
                action_id = np.random.choice(
                    self.output_dim, p=np.squeeze(probs.detach().cpu().numpy())
                )
